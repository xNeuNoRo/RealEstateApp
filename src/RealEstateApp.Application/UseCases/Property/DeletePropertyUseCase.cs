...
public sealed class DeletePropertyUseCase : IDeletePropertyUseCase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!isOwner)
            return Result.Failure(
                Error.Forbidden("Property.NotOwner", "Solo el agente propietario puede eliminar esta propiedad."));

        if (property.Status != PropertyStatus.Available)
            return Result.Failure(
                Error.Conflict("Property.NotAvailable", "Solo se pueden eliminar propiedades disponibles."));
        ...
        foreach (var img in property.Images)
        {
            ...
            {
                _logger.LogWarning(ex, "Error al eliminar imagen {Url} de propiedad {PropertyId}.", img.Url, property.Id);
            }
        }
        ...
        _logger.LogInformation("Propiedad {PropertyId} eliminada por agente {AgentId}.", request.PropertyId, _currentUser.UserId);
